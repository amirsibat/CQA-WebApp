...
/**
 * Servlet implementation class GetUserQuestions
 */
public class GetUserQuestions extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			//check if name exist in url
			if (uri.indexOf("name") != -1)
			{
				String nickname = uri.substring(uri.indexOf("name") + 5);
				user = user.getUserByNickname(nickname);
				QuestsArray = Question.getTop5QuestsOfUserID(Integer.parseInt(user.getUserID()));
			}
